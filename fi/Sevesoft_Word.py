...
def saving_file():
    file_location = asksaveasfilename(defaultextension="txt", filetypes=[("Tekstitiedostot", "*.txt"), ["All files","*.*"]])
    if not file_location:
        return
    with open(file_location, "w") as file_output:
        text = text_edit.get(1.0, tk.END)
        file_output.write(text)
    root.title(f"MINUN OMA MUISTIO - {file_location}")

def opening_file():
    file_location = askopenfilename(filetypes=[("Tekstitiedostot","*.txt"), ("All files","*,*")])
    if not file_location:
        return
...
button_open = tk.Button(frame_button, text="AVAA TIEDOSTO", command=opening_file)
button_open.grid(row=0, column=0, padx=5, pady=5)

button_save = tk.Button(frame_button, text="TALLENNA NIMELLÄ", command=saving_file)
